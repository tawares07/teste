...
sub work {
	...
	my $statement = qq{UPDATE blastSeq_raphael SET jobstatus = "R" where jobid = ?};
	...
	system("blastp -db ".$db." -query ".$queryFile." -outfmt 6 > /var/www/html/disciplina/webdev20171/raphael/result/".$jobID."_result.txt");
	system("rm $queryFile");

	# change job status to Complete
	my $status = "C";
	my $statement2 = qq{UPDATE blastSeq_raphael SET jobstatus = ? where jobid = ?};
